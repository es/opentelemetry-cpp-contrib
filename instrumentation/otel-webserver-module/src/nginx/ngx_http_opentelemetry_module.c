...
static ngx_int_t ngx_http_opentelemetry_init(ngx_conf_t *cf)
{
    ...
    ngx_writeTrace(cf->cycle->log, __func__, "Starting Opentelemetry Module init"); // log only if tracing enabled
    ...
    ngx_writeTrace(cf->cycle->log, __func__, "Registering handlers for modules in different phases");  // log only if tracing enabled
    ...
    */
    ngx_writeTrace(cf->cycle->log, __func__, "Opentelemetry Module init completed!"); // log only if tracing enabled
